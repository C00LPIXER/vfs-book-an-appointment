...
[ -f debug.keystore ] || "$JDK/bin/keytool" -genkeypair -keystore debug.keystore -storepass android \
  -keypass android -alias vfswatch -keyalg RSA -keysize 2048 -validity 10000 \
  -dname "CN=vfswatch, OU=4indegree, O=AAI, L=Kochi, C=IN" >/dev/null 2>&1
...
if [ "${1:-}" = "install" ]; then
  ...
  adb shell am start -n com.fourindegree.vfswatch/.MainActivity
fi
